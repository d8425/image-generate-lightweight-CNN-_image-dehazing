...
parser.add_argument("-e","--epochs",type = int, default = 826)
parser.add_argument("-se","--start_epoch",type = int, default = 475)
...
def main():
    ...
    saved_epoch = 475
    encoder_lv1.load_state_dict(torch.load(str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(saved_epoch)+ "_encoder_lv1.pkl"),map_location={'cuda:1': 'cuda:0'}))
    encoder_lv2.load_state_dict(torch.load(str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(saved_epoch)+ "_encoder_lv2.pkl"),map_location={'cuda:1': 'cuda:0'}))
    encoder_lv3.load_state_dict(torch.load(str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(saved_epoch)+ "_encoder_lv3.pkl"),map_location={'cuda:1': 'cuda:0'}))

    decoder_lv1.load_state_dict(torch.load(str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(saved_epoch)+ "_decoder_lv1.pkl"),map_location={'cuda:1': 'cuda:0'}))
    decoder_lv2.load_state_dict(torch.load(str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(saved_epoch)+ "_decoder_lv2.pkl"),map_location={'cuda:1': 'cuda:0'}))
    decoder_lv3.load_state_dict(torch.load(str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(saved_epoch)+ "_decoder_lv3.pkl"),map_location={'cuda:1': 'cuda:0'}))
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        train_dataset = NH_HazeDataset(
            hazed_image_files = 'new_dataset/hazy.txt',   # make changes here !
            dehazed_image_files = 'new_dataset/gt.txt',
        ...
        for iteration, images in enumerate(train_dataloader):
            ...
            # using for cutting(DMPHN portion)
            images_lv2_1_1 = images_lv1[:,:,0:int(W/2),:]
            images_lv2_2_1 = images_lv1[:,:,int(W/2):W,:]

            feature_lv3 = encoder_lv3(images_lv3)
            residual_lv3 = decoder_lv3(feature_lv3)

            # using for DMPHN PORTION
            feature_lv2_1 = encoder_lv2(images_lv2_1_1)
            feature_lv2_2 = encoder_lv2(images_lv2_2_1)
            feature_lv2_DMPHN = torch.cat((feature_lv2_1, feature_lv2_2), 2)
            residual_lv2_DMPHN = decoder_lv2(feature_lv2_DMPHN)
            ...
            feature_lv1 = encoder_lv1(images_lv1 + residual_lv2 + residual_lv2_DMPHN)
            dehazed_image = decoder_lv1(feature_lv1 + feature_lv2 + feature_lv2_DMPHN)
            ...
            if epoch%25 == 0:
                torch.save(encoder_lv1.state_dict(),str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(epoch)+"_encoder_lv1.pkl"))
                torch.save(encoder_lv2.state_dict(),str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(epoch)+"_encoder_lv2.pkl"))
                torch.save(encoder_lv3.state_dict(),str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(epoch)+"_encoder_lv3.pkl"))

                torch.save(decoder_lv1.state_dict(),str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(epoch)+"_decoder_lv1.pkl"))
                torch.save(decoder_lv2.state_dict(),str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(epoch)+"_decoder_lv2.pkl"))
                torch.save(decoder_lv3.state_dict(),str('./checkpoints2_new_1120/' + METHOD + "/ep" + str(epoch)+"_decoder_lv3.pkl"))
                print('Model saved!')
