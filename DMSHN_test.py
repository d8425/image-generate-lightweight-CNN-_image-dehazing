...
def main():
    ...
    saved_epoch = 1100
    encoder_lv1.load_state_dict(torch.load(str('./checkpoints2/' + METHOD + "/ep" + str(saved_epoch)+ "_encoder_lv1.pkl"),map_location={'cuda:1': 'cuda:0'}))
    encoder_lv2.load_state_dict(torch.load(str('./checkpoints2/' + METHOD + "/ep" + str(saved_epoch)+ "_encoder_lv2.pkl"),map_location={'cuda:1': 'cuda:0'}))
    encoder_lv3.load_state_dict(torch.load(str('./checkpoints2/' + METHOD + "/ep" + str(saved_epoch)+ "_encoder_lv3.pkl"),map_location={'cuda:1': 'cuda:0'}))

    decoder_lv1.load_state_dict(torch.load(str('./checkpoints2/' + METHOD + "/ep" + str(saved_epoch)+ "_decoder_lv1.pkl"),map_location={'cuda:1': 'cuda:0'}))
    decoder_lv2.load_state_dict(torch.load(str('./checkpoints2/' + METHOD + "/ep" + str(saved_epoch)+ "_decoder_lv2.pkl"),map_location={'cuda:1': 'cuda:0'}))
    decoder_lv3.load_state_dict(torch.load(str('./checkpoints2/' + METHOD + "/ep" + str(saved_epoch)+ "_decoder_lv3.pkl"),map_location={'cuda:1': 'cuda:0'}))
    print('MyEnsembleNet parameters:', sum(param.numel() for param in encoder_lv1.parameters()))
    print('MyEnsembleNet parameters:', sum(param.numel() for param in encoder_lv2.parameters()))
    print('MyEnsembleNet parameters:', sum(param.numel() for param in encoder_lv3.parameters()))
    print('MyEnsembleNet parameters:', sum(param.numel() for param in decoder_lv1.parameters()))
    print('MyEnsembleNet parameters:', sum(param.numel() for param in decoder_lv2.parameters()))
    print('MyEnsembleNet parameters:', sum(param.numel() for param in decoder_lv3.parameters()))
